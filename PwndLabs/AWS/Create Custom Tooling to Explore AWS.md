...
**The lab:** https://pwnedlabs.io/labs/create-custom-tooling-to-explore-aws
...
- AWS CLI
- VSCode with Python and Python Debugger extensions

Start intercepting traffic with Burp suite and configure the terminal to proxy all traffic through Burp:
...
For this lab, the bucket `s3.amazonaws.com/dev.huge-logistics.com` is known. From that same terminal we configured the proxy in, we can begin enumeration:

```bash
aws s3 ls s3://dev.huge-logistics.com --no-sign-request
```

Send the request to Repeater in Burp. In request window on left, `Right click > Extensions > Copy As Python-Requests > Copy as requests` and paste into VSCode, then follow the lab and update the script to the following:
